...
typedef NS_ENUM(NSInteger,HZWavePathType){
    HZWavePathTypeSin,
    HZWavePathTypeCos
};
@interface HZWaveLoadingView ()
...
@implementation HZWaveLoadingView
+ (instancetype)loadingView{
    return [[HZWaveLoadingView alloc] initWithFrame:CGRectMake(0, 0, 40, 31)];
}
...
- (void)setupSubviews{
    ...
    self.waveHeight = self.bounds.size.height * 0.5;
...
- (void)statLoading{
    ...
    position.y = position.y - CGRectGetHeight(self.bounds) - 10;
    ...
    [self.waveCosLayer addAnimation:animation forKey:@"positonWave"];
}
- (void)updateWave:(CADisplayLink *)displayLink{
    self.phase += self.phaseShift;
    self.waveSinLayer.path = [self createWavePathWithType:HZWavePathTypeSin].CGPath;
    self.waveCosLayer.path = [self createWavePathWithType:HZWavePathTypeCos].CGPath;
}
- (UIBezierPath *)createWavePathWithType:(HZWavePathType)pathType{
    UIBezierPath *wavePath = [UIBezierPath bezierPath];
    CGFloat endX = 0;
    for (CGFloat x = 0; x < self.waveWidth + 1; x += 1) {
        endX = x;
        CGFloat y = 0;
        if (pathType == HZWavePathTypeSin) {
